...
| `long_run_cap_mi` coach override | long run scales to the cap (e.g. 18 mi) by `cap × week/peak`, **over** the 3 h / share caps | **per-athlete override** *(off by default)* | Off by default — `daniels_long_run`'s `min(3 h, 18, share)` governs. When a coach sets the cap on a **monitored** athlete (via `ManualOverride`), the long run builds toward it as a fraction of peak, deliberately exceeding the time-on-feet / weekly-share safety caps (e.g. a slow runner reaching 18 mi past the 3 h window). Never *reduces* the otherwise-recommended long run. Surfaced as a `coach override:` plan flag. |
...
8. **Strides cadence** — the house default is **one** light stride session per week, following **Hanson** (easy/recovery days are kept *truly easy*; the fast running lives in the structured SOS speed sessions, not bolted onto easy runs). This is intentionally lighter than a literal Daniels Phase I, which can add strides to most E days (p.122/126: "all other days are E runs (+ strides on 3 days)"), and than Pfitzinger's "+ speed" days (which are unaffected — Pfitz plans render from his verbatim grids, where strides are hand-placed). `assemble_week(stride_days=1)` plus the `min(stride_days, easy_days − 1)` clamp means at most one easy run carries strides and at least one plain easy/recovery run always remains.
