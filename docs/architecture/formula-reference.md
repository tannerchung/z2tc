...
| `weekly_volumes` ramp (function default) | hold ~3 wk, +`min(days,10)` mi, recovery wk every 4th | **verbatim** | Daniels p.219 (printed 205): *"increasing weekly mileage about every 4th week … by 1 mile for every running session … don't increase by more than 10 miles"*; recovery week = Pfitzinger ch.3. This is the function's default and the single-author reference contract. |
| `aggressive_volume_ramp` coach override (`hold_weeks=1`) | +`min(days,10)` mi **every** week (recovery wk every 4th), capped at peak | **per-athlete override** *(off by default)* | Off by default — the Daniels generator uses the verbatim 3-week hold above. A coach can opt a **monitored** athlete in (set via a `ManualOverride` event); the generator then calls `weekly_volumes(hold_weeks=1)` so the build reaches and **holds** the demonstrated peak inside a fixed block instead of only grazing it. Step size (+1 mi/day, ≤10) and the peak ceiling are unchanged. Surfaced as a `coach override:` plan flag. |
| `long_run_cap_mi` / `long_run_peak_weeks` coach override | long run **ramps progressively** to the cap (e.g. 18 mi) and sits at it only for the final `long_run_peak_weeks` build weeks (default 3), **over** the 3 h / share caps | **per-athlete override** *(off by default)* | Off by default — `daniels_long_run`'s `min(3 h, 18, share)` governs. When a coach sets the cap on a **monitored** athlete (via `ManualOverride`), `coach_long_run_targets` climbs the long run from the book week-1 distance up to the cap, reaching it only on the last few non-down build long runs (down weeks stay book cutbacks). This is the Higdon-style "ramp up, peak once or twice, then taper" shape — *not* a jump to the cap held for most of the block — and deliberately exceeds the time-on-feet / weekly-share safety caps near the peak. Surfaced as a `coach override:` plan flag. |
| `quality_long_runs_race_prep_only` coach override | Base/Threshold long runs stay **easy/aerobic**; quality long runs only in race-prep, alternated with easy longs | **per-athlete override** *(off by default)* | Off by default — the Daniels generator rotates a quality long run (Q1) every build week. Daniels' 2Q assumes ~6 days and caps/recovers Q1; on a **4-day** load the native 4-day model (Higdon Novice) keeps *every* long run easy and does pace work as a separate midweek run. With the override the midweek **Q2** carries the weekly quality, threshold long runs are easy, and quality long runs (MP blocks, fast-finish, race-practice) are confined to the race-specific block and spaced. Surfaced as a `coach override:` plan flag. |
| `strides_per_phase` coach override | cap on stride **weeks per phase** (default `STRIDES_PER_PHASE` = 3) | **per-athlete override** *(default = 3)* | Lowers the per-phase stride budget (item 8 below) for a given athlete — e.g. 2 instead of 3. Falls back to the `STRIDES_PER_PHASE` house default when unset. |
| `weekday_quality_sessions` coach override | midweek quality sessions in a **build week** (pure default 1 = Daniels 2Q) | **per-athlete override** *(pure default = 1; the **club engine** defaults it to 2 — see `engine/plan/club.py`)* | `2` adds a **second** midweek quality run — a continuous goal-pace **Race-pace run** (`marathon_pace_run`, ≤ M cap / 6 mi) on the second-quality day (`SECOND_QUALITY_DAY_BY_COUNT`; at 4 days **Thursday**, with Tuesday as the primary Q2). Needs ≥ 4 run days. Applied in the **Threshold** and **Race Prep** phases; the **Base** phase only carries it when `base_quality_ramp` is set (below). Skipped on **down weeks** and on weeks the **long run is itself a quality session**, so a quality-long week stays at two hard sessions (long + Q2), not three. On a 4-day `quality_long_runs_race_prep_only` load this gives threshold weeks two real quality runs (a threshold session + a race-pace run) and seeds **race-practice** work into the threshold phase. Surfaced as a plan flag. |
| `base_quality_ramp` (club policy) | ease the second quality into the **Base** phase | **off in the pure engine; the club engine turns it on for two-quality athletes** | Pure Daniels keeps Phase I (Base) aerobic. When set (with `weekday_quality_sessions ≥ 2`), the second quality is **eased in** — only the back half of Base carries it; early Base keeps a single, gentle Reps→threshold Q2 (`BASE_Q2`) — so the block opens at one effort and ramps to two before the Threshold phase. |
...
7. **`weekly_volumes` ramp cadence** — the function defaults to the Daniels **3-week hold** (verbatim, p.219) and **all generators use it as-is by default**. A faster ramp (`hold_weeks=1`: +1 mi per running day **every** week, recovery week every 4th, capped at peak) is a **per-athlete coach override** (`aggressive_volume_ramp`), not a default-engine behavior — it lets a monitored athlete reach and **hold** the peak inside a fixed block. A build that can't reach an aggressive `P` inside the block is **flagged** (`peak not reached`) rather than over-ramped. The coach-facing **re-entry start**, **recommended P**, **goal feasibility**, **race-time prediction**, and **Table 15.1 break** model now live in [`engine/readiness.py`](../../engine/readiness.py) (see [athlete-readiness.md §10-12](athlete-readiness.md)).
8. **Strides cadence** — strides are an **economy touch, not a weekly fixture**: the Daniels generator budgets at most `STRIDES_PER_PHASE` (**3**, lowerable per athlete via the `strides_per_phase` override) stride weeks per phase, front-loaded, and never on a down/recovery week. Within a stride week, `assemble_week(stride_days=1)` plus the `min(stride_days, easy_days − 1)` clamp puts strides on **one** easy run and always leaves a plain easy/recovery run. This follows **Hanson** (easy/recovery days are kept *truly easy*; the fast running lives in the structured SOS sessions) and is intentionally lighter than a literal Daniels Phase I, which can add strides to most E days (p.122/126: "all other days are E runs (+ strides on 3 days)"), and than Pfitzinger's "+ speed" days (unaffected — Pfitz plans render from his verbatim grids, where strides are hand-placed). With `weekday_quality_sessions = 2` a build week has only one easy weekday run, so the `easy_days − 1` clamp leaves it plain — budgeted strides then fall in the Base phase, where there is easy-day headroom.
9. **Weekly day spacing** (`RUN_DAYS_BY_COUNT` / `QUALITY_DAY_BY_COUNT`) — the day maps bracket the Saturday long run with rest at lower frequencies, following **Higdon Novice** (Mon rest · Tue–Thu runs · **Fri rest** · Sat long · Sun cross/rest — the long run is preceded and followed by rest). So the **4-day** week is `Mon · Tue · Thu · Sat` (Fri rest) with Q2 on **Tuesday**, and the **3-day** week is `Mon · Wed · Sat` — a growing easy run never lands the day before the long run, and the midweek Q2 stays maximally separated from it. This is *not* Hanson's deliberately fatigued long run (a 6-day, cumulative-fatigue model); at 4 days the Higdon bracketing is the right fit. 5+ day weeks keep the Daniels/Pfitzinger midweek-`Wednesday` Q2.
