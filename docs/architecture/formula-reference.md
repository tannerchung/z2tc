...
8. **Strides cadence** — strides are an **economy touch, not a weekly fixture**: the Daniels generator budgets at most `STRIDES_PER_PHASE` (**3**) stride weeks per phase, front-loaded, and never on a down/recovery week. Within a stride week, `assemble_week(stride_days=1)` plus the `min(stride_days, easy_days − 1)` clamp puts strides on **one** easy run and always leaves a plain easy/recovery run. This follows **Hanson** (easy/recovery days are kept *truly easy*; the fast running lives in the structured SOS sessions) and is intentionally lighter than a literal Daniels Phase I, which can add strides to most E days (p.122/126: "all other days are E runs (+ strides on 3 days)"), and than Pfitzinger's "+ speed" days (unaffected — Pfitz plans render from his verbatim grids, where strides are hand-placed).
